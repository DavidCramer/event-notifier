...
/**
 * EVENT_NOTIFY Page class for creating admin/settings pages.
 * @package evenote\ui
 * @author  David Cramer
 */
class page extends box implements \evenote\data\save {
	...
	public function set_assets() {
		...
		$this->assets['style']['page'] = $this->url . 'assets/css/page' . EVENT_NOTIFY_ASSET_DEBUG . '.css';
	...
	public function set_attributes() {
		...
		if ( empty( $this->struct['full_width'] ) ) {
			$this->attributes['class'] .= ' evenote-half-page';
		}
	...
	/**
	 * get a EVENT_NOTIFY config store key
	 * @since 1.0.0
	 * @access public
	 * @return string $store_key the defiuned option name for this EVENT_NOTIFY object
	 */
	public function store_key() {
		...
		if ( ! empty( $this->struct['store_key'] ) ) {
			...
			$store_key = 'evenote-' . $this->type . '-' . sanitize_text_field( $this->slug );
		}
