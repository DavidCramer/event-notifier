 ...
 * @author  David Cramer
 */
class box extends panel implements \evenote\data\save, \evenote\data\load {
	...
	public function init() {
		...
		$data = evenote()->request_vars( 'post' );
		if ( isset( $data[ 'evenoteNonce_' . $this->id() ] ) && wp_verify_nonce( $data[ 'evenoteNonce_' . $this->id() ], $this->id() ) ) {
			$this->save_data();
		} else {
			// load data normally
	...
	public function set_assets() {
		...
		$this->assets['script']['baldrick'] = array(
			'src'  => $this->url . 'assets/js/jquery.baldrick' . EVENT_NOTIFY_ASSET_DEBUG . '.js',
			'deps' => array( 'jquery' ),
		);
		$this->assets['script']['evenote-ajax'] = array(
			'src'  => $this->url . 'assets/js/ajax' . EVENT_NOTIFY_ASSET_DEBUG . '.js',
			'deps' => array( 'baldrick' ),
		);
		$this->assets['style']['evenote-ajax']  = $this->url . 'assets/css/ajax' . EVENT_NOTIFY_ASSET_DEBUG . '.css';
	...
	public function set_attributes() {
		...
		$action = evenote()->request_vars( 'server' );
		$attributes = array(
			'enctype'  => 'multipart/form-data',
			'method'   => 'POST',
			'class'    => 'evenote-ajax evenote-' . $this->type,
			'data-evenote' => $this->slug,
			'action'   => $action['REQUEST_URI'],
		);
		if ( ! empty( $this->struct['static'] ) ) {

			$attributes = array(
				'class'    => 'evenote-' . $this->type,
				'data-evenote' => $this->slug,
			);
		}
	...
	public function render() {
		...
		$output .= wp_nonce_field( $this->id(), 'evenoteNonce_' . $this->id(), true, false );
