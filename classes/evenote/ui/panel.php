...
/**
 * EVENT_NOTIFY panel. a holder to contain sections. a panel with multiple sections creates a tabbed interface to switch between sections areas.
 *
 * @package evenote\ui
 * @author  David Cramer
 */
class panel extends \evenote\data\data {
	...
	public function set_assets() {
		...
		$this->assets['script']['panel'] = $this->url . 'assets/js/panel' . EVENT_NOTIFY_ASSET_DEBUG . '.js';
		$this->assets['style']['panel']  = $this->url . 'assets/css/panel' . EVENT_NOTIFY_ASSET_DEBUG . '.css';
	...
	public function render() {
		...
		if ( $this->child_count() > 0 ) {
			...
			$output .= '<div id="panel-' . esc_attr( $this->id() ) . '" class="evenote-' . esc_attr( $this->type ) . '-inside ' . esc_attr( $this->wrapper_class_names() ) . '">';
			// render a lable
			$output .= $this->label();
			// render a desciption
	 ...
	 * @access public
	 *
	 * @param evenote Object to test if it is to be rendered in a section
	 *
	...
	public function wrapper_class_names() {
		...
		$wrapper_class_names = array(
			'evenote-panel-inside',
		);

		if ( $this->child_count() > 1 ) {
			$wrapper_class_names[] = 'evenote-has-tabs';

			if ( ! empty( $this->struct['top_tabs'] ) ) {
				$wrapper_class_names[] = 'evenote-top-tabs';
			}
		}
	...
	public function label() {
		...
		if ( ! empty( $this->struct['label'] ) ) {
			$output .= '<div class="evenote-' . esc_attr( $this->type ) . '-heading"><h3 class="evenote-' . esc_attr( $this->type ) . '-title">' . esc_html( $this->struct['label'] ) . '</h3></div>';
		}
	...
	public function description() {
		...
		if ( ! empty( $this->struct['description'] ) ) {
			$output .= '<div class="evenote-' . esc_attr( $this->type ) . '-heading"><p class="evenote-' . esc_attr( $this->type ) . '-subtitle description">' . esc_html( $this->struct['description'] ) . '</p></div>';
		}
	...
	public function navigation() {
		...
		if ( $this->child_count() > 1 ) {
			...
			$output .= '<ul class="evenote-' . esc_attr( $this->type ) . '-tabs evenote-panel-tabs">';
			$active = 'true';
			foreach ( $this->child as $child ) {
				if ( $this->is_section_object( $child ) ) {
	...
	private function tab_label( $child, $active ) {
		...
		$output .= '<a href="#' . esc_attr( $child->id() ) . '" data-parent="' . esc_attr( $this->id() ) . '" class="evenote-tab-trigger">' . $label . '</a>';
		$output .= '</li>';
	...
	public function panel_section() {
		...
		$output .= '<div class="evenote-' . esc_attr( $this->type ) . '-sections evenote-sections">';
	...
	protected function set_active_styles() {
		$style = '#panel-' . $this->id() . ' > .evenote-panel-tabs > li[aria-selected="true"] a {box-shadow: 3px 0 0 ' . $this->base_color() . ' inset;}';
		$style .= '#panel-' . $this->id() . '.evenote-top-tabs > .evenote-panel-tabs > li[aria-selected="true"] a {	box-shadow: 0 3px 0 ' . $this->base_color() . ' inset;}';
