...
/**
 * Base EVENT_NOTIFY Control class.
 *
 * @since       1.0.0
 */
class control extends \evenote\data\data {
	...
	public static function register( $slug, $object, $parent = null ) {
		...
		if ( empty( $object['type'] ) || ! evenote()->is_callable( 'control\\' . $object['type'] ) ) {
			$object['type'] = 'text';
		}
	...
	public function set_assets() {
		$this->assets['style']['controls'] = $this->url . 'assets/css/control' . EVENT_NOTIFY_ASSET_DEBUG . '.css';
		parent::set_assets();
	}
	...
	public function render() {
		...
		$output = '<div id="' . esc_attr( $this->id() ) . '" class="evenote-control evenote-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '">';

		$output .= $this->label();
		$output .= '<div class="evenote-control-input">';
		$output .= $this->input();
		$output .= '</div>';
	...
	public function label() {
		...
		if ( isset( $this->struct['label'] ) ) {
			$output .= '<label for="' . esc_attr( $this->id() ) . '-control"><span class="evenote-control-label">' . esc_html( $this->struct['label'] ) . '</span></label>';
		}
